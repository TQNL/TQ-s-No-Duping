...
execute if data storage tqs_no_duping:dupe_settings {gravity_blocks:0b} as @e[type=falling_block] at @s if dimension the_end if block ~ ~-1 ~ obsidian run kill @s

execute if data storage tqs_no_duping:dupe_settings {tripwire_hook:0b} as @e[type=item] at @s if items entity @s contents tripwire_hook if block ~ ~ ~ tripwire_hook run kill @s
